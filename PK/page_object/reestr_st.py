...
class LocatorReestr:
    ...
    LOCATOR_TYPE_2 = (By.XPATH, '(//span[contains(text(), "СТАЦ_КС_ДС_31.05.2021")])[3]')
    ...
    LOCATOR_UNIT_4 = (By.XPATH, '//div[@name="ComboItemsList_DS_DEPS_DEFAULT_DP_KIND_ID_FilterItem"]//span[contains(text(), "Стационар")]')
...
class ReestrSt(BasePage):
    ...
    def reestr(self):
        start_date, end_date = ReestrSt.get_data()
        try:
            print('🔽 Модуль: Формирование реестра (Стационара)')
            ...
            self.get_screenshots('Results/Results_sc/РеестрСтационара_Приморья.png')
            print('    ❗️ Возникла ошибка в модуле - "Реестры (Стационара)":', (str(e)[:100]))
            self.driver.refresh()
            pass
